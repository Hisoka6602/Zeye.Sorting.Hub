...
- `20260323045038_UseExternalProvidedParcelId.cs`：Parcel 主表主键生成策略迁移（移除 Parcels.Id 自动生成，改为外部传入）；已同步补齐 MySQL 与 SQL Server 的 Identity 注解变更元数据。
...
- 已复核 `20260323045038_UseExternalProvidedParcelId` 的跨 Provider 兼容性：迁移脚本已补齐 SQL Server `SqlServer:Identity` 注解的 Up/Down 对应变更，不再仅反映 MySQL Identity 注解变化。
