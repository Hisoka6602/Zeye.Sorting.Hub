...
│   │   ├── DatabaseAutoTuningHostedService.cs（数据库自动调谐托管服务（慢查询分析、阈值告警、L2 可控执行、L3 闭环自治执行验证回退与变更审计））
...
│   │   │   ├── SlowQueryAutoTuningPipeline.cs（慢查询采集、TopN 聚合、阈值告警（含基础防抖）与 L2/L3 结构化建议编排管道）
