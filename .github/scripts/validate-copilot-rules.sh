...
ensure_pr_diff_ready() {
  ...
  if ! git show-ref --verify --quiet "refs/remotes/origin/${GITHUB_BASE_REF}" \
    && ! git fetch --no-tags --prune origin "${GITHUB_BASE_REF}:refs/remotes/origin/${GITHUB_BASE_REF}" >/dev/null 2>&1; then
