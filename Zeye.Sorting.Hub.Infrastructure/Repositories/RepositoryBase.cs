...
namespace Zeye.Sorting.Hub.Infrastructure.Repositories {
    ...
    /// 2) 每次变更操作自动调用 SaveChangesAsync 提交；跨多个仓储的原子事务需改用 TransactionScope；
    /// 3) 通过 RepositoryResult 返回错误信息，隔离异常，不影响调用链；
    /// 4) Logger 由派生类通过 <c>LogManager.GetCurrentClassLogger()</c> 创建后传入，确保日志来源类名正确。
    /// </summary>
    public abstract class RepositoryBase<TEntity, TContext>
        ...
        /// <param name="logger">NLog 日志器（由派生类传入，确保日志来源类名为实际仓储类名）。</param>
        ...
        public virtual async Task<RepositoryResult> AddAsync(TEntity entity, CancellationToken cancellationToken) {
            ...
            try {
                ...
                Logger.Warn(ex, "新增实体操作被取消，实体类型={EntityType}", typeof(TEntity).Name);
                return RepositoryResult.Fail("操作已取消");
            }
            catch (Exception ex) {
                Logger.Error(ex, "新增实体失败，实体类型={EntityType}", typeof(TEntity).Name);
        ...
        public virtual async Task<RepositoryResult> AddRangeAsync(
            ...
            try {
                ...
                Logger.Warn(ex, "批量新增操作被取消，实体类型={EntityType}", typeof(TEntity).Name);
                return RepositoryResult.Fail("操作已取消");
            }
            catch (Exception ex) {
                Logger.Error(ex, "批量新增失败，实体类型={EntityType}", typeof(TEntity).Name);
        ...
        public virtual async Task<RepositoryResult> UpdateAsync(TEntity entity, CancellationToken cancellationToken) {
            ...
            try {
                ...
                Logger.Warn(ex, "更新实体操作被取消，实体类型={EntityType}", typeof(TEntity).Name);
                return RepositoryResult.Fail("操作已取消");
            }
            catch (Exception ex) {
                Logger.Error(ex, "更新实体失败，实体类型={EntityType}", typeof(TEntity).Name);
        ...
        public virtual async Task<RepositoryResult> RemoveAsync(TEntity entity, CancellationToken cancellationToken) {
            ...
            try {
                ...
                Logger.Warn(ex, "删除实体操作被取消，实体类型={EntityType}", typeof(TEntity).Name);
                return RepositoryResult.Fail("操作已取消");
            }
            catch (Exception ex) {
                Logger.Error(ex, "删除实体失败，实体类型={EntityType}", typeof(TEntity).Name);
