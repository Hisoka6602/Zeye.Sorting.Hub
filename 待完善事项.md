...
1. ~~在 CI 中增加"下周期分表预建校验"门禁（按 `PrebuildWindowHours` 自动检查），将当前运行期守卫升级为发布前自动门禁。~~ **已实现**：新增 `sharding-prebuild-gate` 门禁（门禁9），验证 `PrebuildWindowHours >= 24` 且分表预建逻辑存在于 `DatabaseInitializerHostedService`。
...
3. ~~将"月度回滚演练 / 季度灾备升降级演练"接入流水线门禁，自动校验演练记录完备性。~~ **已实现**：`drill-record-gate`（门禁8）已升级为强制阻断，`drill-records/` 目录已创建并包含首份季度演练记录。
...
2. ~~可将《长期运行优化与热更新支持清单》中的年度验收项逐条映射到 CI/发布流水线自动门禁，减少人工核对成本。~~ **已实现**：`stability-gates.yml` 现已包含 10 个并行门禁，覆盖构建测试、配置合法性、隔离器边界、回滚资产、健康探针、契约兼容性、蓝绿部署、演练记录（强制阻断）、分表预建校验、迁移归档验证。
