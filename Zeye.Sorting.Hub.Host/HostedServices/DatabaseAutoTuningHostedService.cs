...
namespace Zeye.Sorting.Hub.Host.HostedServices {
    ...
    public sealed class DatabaseAutoTuningHostedService : BackgroundService {
        ...
        public DatabaseAutoTuningHostedService(
            ...
            _dialect = dialect;
            _pipeline = pipeline;
            _analyzeIntervalSeconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("AnalyzeIntervalSeconds"), 30);
            _maxExecuteActionsPerCycle = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:MaxExecuteActionsPerCycle"), 2);
            _actionExecutionTimeoutSeconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:ActionExecutionTimeoutSeconds"), 60);
            _skipExecutionDuringPeak = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:SkipExecutionDuringPeak"), true);
            _peakStartTime = AutoTuningConfigurationReader.GetTimeOfDayOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:PeakStartLocalTime"), new TimeSpan(8, 0, 0));
            _peakEndTime = AutoTuningConfigurationReader.GetTimeOfDayOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:PeakEndLocalTime"), new TimeSpan(21, 0, 0));
            _enableDangerousActionIsolator = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:Isolator:EnableGuard"), true);
            _allowDangerousActionExecution = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:Isolator:AllowDangerousActionExecution"), false);
            _enableActionDryRun = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:Isolator:DryRun"), false);
            _gradualSwitchDryRunCycles = AutoTuningConfigurationReader.GetNonNegativeIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Execution:Isolator:GradualSwitchDryRunCycles"), 0);
            _enableAutoRollback = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:EnableAutoRollback"), true);
            _whitelistedTables = LoadWhitelistedTables(configuration.GetSection(AutoTuningConfigurationReader.BuildAutonomousKey("Execution:WhitelistedTables")));
            if (_whitelistedTables.Count == 0) {
                NLogLogger.Warn("自动调优执行白名单为空：当前将阻断所有候选表自动动作。");
            }
            _baselineCommandTimeoutSeconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("BaselineCommandTimeoutSeconds"), 30);
            _baselineMaxRetryCount = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("BaselineMaxRetryCount"), 5);
            _baselineMaxRetryDelaySeconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("BaselineMaxRetryDelaySeconds"), 10);
            _configuredCommandTimeoutSeconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, $"{PerformanceConfigPrefix}:CommandTimeoutSeconds", 30);
            _configuredMaxRetryCount = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, $"{PerformanceConfigPrefix}:MaxRetryCount", 5);
            _configuredMaxRetryDelaySeconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, $"{PerformanceConfigPrefix}:MaxRetryDelaySeconds", 10);
            _configuredSlowQueryThresholdMilliseconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("SlowQueryThresholdMilliseconds"), 500);
            _configuredTriggerCount = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("TriggerCount"), 3);
            _configuredMaxSuggestionsPerCycle = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("MaxActionsPerCycle"), 3);
            _configuredAlertP99Milliseconds = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("AlertP99Milliseconds"), 500);
            _configuredAlertTimeoutRatePercent = AutoTuningConfigurationReader.GetNonNegativeDecimalOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("AlertTimeoutRatePercent"), 1m);
            _configuredAlertDeadlockCount = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutoTuningKey("AlertDeadlockCount"), 1);
            _enableFullAutomation = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("EnableFullAutomation"), true);
            _policyMinTableHeatCalls = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Policy:MinTableHeatCalls"), 10);
            _policyMaxRiskScore = AutoTuningConfigurationReader.GetDecimalInRangeOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Policy:MaxRiskScore"), 0.85m, 0m, 1m);
            _policyPeakMaxRiskScore = AutoTuningConfigurationReader.GetDecimalInRangeOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Policy:PeakMaxRiskScore"), 0.45m, 0m, 1m);
            _policyPeakMaxTableHeatCalls = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Policy:PeakMaxTableHeatCalls"), 50);
            _enableAutoValidation = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:EnableAutoValidation"), true);
            _validationDelayCycles = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:DelayCycles"), 1);
            _validationP95IncreasePercent = AutoTuningConfigurationReader.GetNonNegativeDecimalOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:P95IncreasePercent"), 5m);
            _validationP99IncreasePercent = AutoTuningConfigurationReader.GetNonNegativeDecimalOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:P99IncreasePercent"), 10m);
            _validationErrorRateIncreasePercent = AutoTuningConfigurationReader.GetNonNegativeDecimalOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:ErrorRateIncreasePercent"), 0.5m);
            _validationTimeoutRateIncreasePercent = AutoTuningConfigurationReader.GetNonNegativeDecimalOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:TimeoutRateIncreasePercent"), 0.5m);
            _validationDeadlockIncreaseCount = AutoTuningConfigurationReader.GetNonNegativeIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:DeadlockIncreaseCount"), 1);
            _enableCapacityPrediction = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("CapacityPrediction:EnableCapacityPrediction"), true);
            _capacityProjectionDays = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("CapacityPrediction:ProjectionDays"), 7);
            _capacityGrowthAlertRows = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("CapacityPrediction:GrowthAlertRows"), 50000);
            _capacityHotLayeringRows = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("CapacityPrediction:HotLayeringRows"), 200000);
            _severeRollbackP99IncreasePercent = AutoTuningConfigurationReader.GetNonNegativeDecimalOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:SevereRollback:P99IncreasePercent"), 25m);
            _severeRollbackTimeoutIncreasePercent = AutoTuningConfigurationReader.GetNonNegativeDecimalOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:SevereRollback:TimeoutRateIncreasePercent"), 2m);
            _pauseActionCyclesOnRegression = AutoTuningConfigurationReader.GetPositiveIntOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:PauseActionCyclesOnRegression"), 2);
            _enablePlanProbe = AutoTuningConfigurationReader.GetBoolOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:PlanProbe:Enable"), true);
            _planProbeSampleRate = AutoTuningConfigurationReader.GetDecimalClampedOrDefault(configuration, AutoTuningConfigurationReader.BuildAutonomousKey("Validation:PlanProbe:SampleRate"), 1m, 0m, 1m);
